...
The `database_creator.py` script automates the process of creating, preprocessing, and optionally simplifying an SQLite database for the TEMOA-NO4 project. The script executes in four primary steps controlled by Boolean flags:
...
- **SQLite Database File:** `TEMOA_NO4.sqlite`
- **SQL Script(s):** `TEMOA_NO4.sql`
...
- **Functionality:**
  - Checks if the database file (`TEMOA_NO4.sqlite`) exists.
...
- **Functionality:**
  - Iterates over the `sql_modules` list (e.g., `TEMOA_NO4.sql`).
